...
export function RowActions<T>({ row, actions }: Props<T>) {
  ...
  const getVariantClasses = (variant?: ActionVariant) => {
    switch (variant) {
      case "destructive":
        return "text-destructive group-hover:text-destructive/80 hover:bg-destructive/10";
      case "edit":
        return "text-teal-700 group-hover:text-teal-700/80 hover:bg-teal-700/10";
      case "view":
      default:
        return "text-gray-500 group-hover:text-gray-500/80 hover:bg-gray-500/10";
    }
  };
